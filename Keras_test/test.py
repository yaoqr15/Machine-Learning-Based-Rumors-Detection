...
tmp = pd.read_excel("test.xls", header=None)
pos['label']=1
neg['label']=0
all = pos.append(neg, ignore_index=True)

all['word']=list(map(lambda s: list(jieba.cut(s,cut_all=False)), all[0]))
tmp['word']=list(map(lambda s: list(jieba.cut(s,cut_all=False)), tmp[0]))
